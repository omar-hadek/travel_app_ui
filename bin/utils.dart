...
Future<String> bumpPackageVersion(VersionBumpType type) async {
  ...
  final String updatedPubspec = pubspec.replaceFirstMapped(
      versionRegExp, (Match match) => '${match[1]}$newVersion');
...
enum BuildPlatform {
  ...
  static BuildPlatform fromName(String? name) =>
      values.asNameMap()[name] ?? BuildPlatform.android;
...
Future<void> runCmdAction(CmdAction action,
...
Future<dynamic> execCmdAction(CmdAction action,
    {required Map<String, String> environment}) async {
  stdout.writeln('Running ${action.cmd}');
  final List<String> args = action.cmd.split(' ');
