...
void main(List<String> arguments) async {
  final DotEnv env = DotEnv(includePlatformEnvironment: true)
    ..load(<String>['${Platform.script.path}/../.env']);
  ...
  for (final CmdAction action in <CmdAction>[
    ...
    if (platform.contains(BuildPlatform.ios.name)) ...<CmdAction>[
      const CmdAction(
          'fvm flutter build ios --flavor prod --dart-define=env.mode=prod --release --no-codesign'),
...
List<CmdAction> _analysisChecks() {
  const String dartMetricCommand =
      'fvm flutter pub run dart_code_metrics:metrics';
  ...
  return <CmdAction>[
    ...
    const CmdAction(
        '$dartMetricCommand check-unnecessary-nullable lib --fatal-found'),
  ];
}
